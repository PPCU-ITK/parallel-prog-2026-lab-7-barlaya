...
void mpi_setup(int *imax, int *jmax) {
	...
	if (my_rank == 0) {
		std::cout << "running with" << dims[0] << " x " << dims[1] << " processes\n";
	}
	...
	MPI_Cart_shift(cart_comm, 0, 1, &prev_x, &next_x);
	MPI_Cart_shift(cart_comm, 1, 1, &prev_y, &next_y);
	...
	*jmax = jmax_full / dims[1];
	...
	//gbl_i_begin = imax_full / my_rank_x;
	//gbl_j_begin = jmax_full / my_rank_y;	
	gbl_i_begin = my_rank_x * (imax_full / dims[0]);
	gbl_j_begin = my_rank_y * (jmax_full / dims[1]);
...
void exchange_halo(int imax, int jmax, double *arr) {
	...
	MPI_Sendrecv(&arr[(imax+2)],1, column, next_x, 0, 
